...
## 16. tool_call、dispatch_tool、tool_result 的关系
...
```text
tool_call：模型发出的工具调用请求，说明要调用哪个工具、传什么参数。
dispatch_tool：程序根据工具名和参数执行真实工具。
tool_result：工具执行后的结果，被写回 messages，作为下一轮模型推理的上下文。
```
...
dispatch_tool 不是大模型的方法。
dispatch_tool 是项目里的工具分发函数。

tool_result 不是最终回答。
tool_result 是工具返回的数据，需要再交给大模型组织成最终回答。
```
...
```text
M6 ReAct 的核心是围绕 messages 做循环。

每轮先把 messages 和 tools=ALL_TOOLS 发给模型。模型根据上下文和工具 schema 判断是否需要工具。

如果需要工具，模型返回 tool_calls。tool_call 里包含工具名、参数和 tool_call_id。程序解析 tool_call，通过 dispatch_tool 执行真实工具，得到 tool_result。

然后程序把 tool_result 包装成 role="tool" 的消息写回 messages，并用 tool_call_id 对应前面的工具调用。

下一轮再次把新的 messages 发给模型。模型继续判断信息是否足够：不够就继续返回 tool_calls，够了就返回普通 content。代码看到没有 tool_calls，就把 content 作为最终答案返回。
```
